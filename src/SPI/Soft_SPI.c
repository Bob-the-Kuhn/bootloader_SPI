...
void k_delay(const uint32_t ms);
  ...
  void spiBegin(void) {
    ...
    GPIO_CONFIG_OUTPUT(PORTC, 10, GPIO_NO_PULL_UP_DOWN, GPIO_OUTPUT_PUSH_PULL, GPIO_OUTPUT_VERY_HIGH_SPEED);  // SCK
    GPIO_CONFIG_INPUT(PORTC, 11, GPIO_NO_PULL_UP_DOWN);                                                       // MISO
    GPIO_CONFIG_OUTPUT(PORTC, 12, GPIO_NO_PULL_UP_DOWN, GPIO_OUTPUT_PUSH_PULL, GPIO_OUTPUT_VERY_HIGH_SPEED);  // MOSI
    GPIO_CONFIG_OUTPUT(PORTA,4, GPIO_NO_PULL_UP_DOWN, GPIO_OUTPUT_PUSH_PULL, GPIO_OUTPUT_VERY_HIGH_SPEED);    // SDSS
  ...
  //uint8_t SOFT_SPI_STM32_SpiTransfer_Mode_0(uint8_t b) { // using Mode 0     
    ...
    for (uint8_t bits = 8; bits--;) {                                        // 
      ...
      WRITE_SD_SCK_PIN_SET;                                            // WRITE_SD_SCK_PIN_SET
        ...
        WRITE_SD_SCK_PIN_SET;                                                 // WRITE_SD_SCK_PIN_SET (delay)
        data_out |= (SD_MISO_PIN_READ != 0);                                  
        WRITE_SD_SCK_PIN_SET;                                                 // WRITE_SD_SCK_PIN_SET (delay)        data_out |= (SD_MISO_PIN_READ != 0);                                  
        WRITE_SD_SCK_PIN_SET;                                                 // WRITE_SD_SCK_PIN_SET (delay)
        data_out |= (SD_MISO_PIN_READ != 0);                                  
        WRITE_SD_SCK_PIN_SET;                                                 // WRITE_SD_SCK_PIN_SET (delay)
        data_out |= (SD_MISO_PIN_READ != 0);                                  

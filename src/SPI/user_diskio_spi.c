...
void SPI_Timer_On(uint32_t waitTicks) {
    spiTimerTickStart = k_ticks();
    spiTimerTickDelay = waitTicks;
}

uint8_t SPI_Timer_Status() {
    return ((k_ticks() - spiTimerTickStart) < spiTimerTickDelay);
}
...
)
{
	...
	waitSpiTimerTickStart = k_ticks();
	waitSpiTimerTickDelay = (uint32_t)wt;
	do {
		d = xchg_spi(0xFF);
		/* This loop takes a time. Insert rot_rdq() here for multitask envilonment. */
	} while (d != 0xFF && ((k_ticks() - waitSpiTimerTickStart) < waitSpiTimerTickDelay));	/* Wait for card goes ready or timeout */

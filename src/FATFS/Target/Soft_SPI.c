...
// ------------------------
  ...
  uint8_t spiRec() {
    __disable_irq();                                               // No interrupts during byte receive
    const uint8_t data = SOFT_SPI_STM32_SpiTransfer_Mode_3(0xFF);
  ...
  void spiSend(uint8_t data) {
    __disable_irq();                         // No interrupts during byte send
    SOFT_SPI_STM32_SpiTransfer_Mode_3(data); // Don't care what is received
    __enable_irq();                           // Enable interrupts
  }
